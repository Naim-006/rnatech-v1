...
const LandingPage = () => {
  const { t, lang, theme, currency, addToCart } = useApp();
      ...
      {featuredProducts.length > 0 && (
        <section className="py-24 max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8 text-center md:text-left">
            <div>
              <h2 className={`text-4xl md:text-6xl font-black mb-4 leading-tight tracking-tighter transition-colors ${theme === 'dark' ? 'text-white' : 'text-slate-950'}`}>
                {lang === 'bn' ? 'ফিচার্ড হার্ডওয়্যার' : 'Hardware Showcase'}
              </h2>
              <p className="text-slate-500 text-xl font-medium uppercase tracking-[0.2em]">Featured Innovation</p>
            </div>
            <Link to="/shop" className="px-8 py-3 rounded-2xl bg-white/5 border border-white/10 font-black text-xs uppercase tracking-widest hover:bg-brand-500 hover:text-white transition-all">
              {lang === 'bn' ? 'সব দেখুন' : 'View Full Arsenal'}
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {featuredProducts.map((p) => (
              <motion.div
                key={p.id}
                whileHover={{ y: -10 }}
                className={`relative group rounded-[3rem] overflow-hidden border transition-all smooth-animate
                  ${theme === 'dark' ? 'bg-slate-950 border-white/5 hover:border-brand-500/50' : 'bg-white border-slate-200 shadow-xl shadow-slate-200/50'}`}
              >
                {/* Floating Tags */}
                <div className="absolute top-6 left-6 z-20 flex flex-col gap-2">
                  {p.tag && (
                    <span className="px-4 py-1.5 bg-brand-600 text-white text-[10px] font-black uppercase tracking-widest rounded-full shadow-lg">
                      {p.tag}
                    </span>
                  )}
                  {p.is_on_sale && (
                    <span className="px-4 py-1.5 bg-rose-500 text-white text-[10px] font-black uppercase tracking-widest rounded-full shadow-lg">
                      {lang === 'bn' ? 'অফার' : 'SALE'}
                    </span>
                  )}
                </div>
                ...
                <div className="aspect-square overflow-hidden bg-white/5 relative">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-8">
                    <button 
                      onClick={(e) => { e.preventDefault(); addToCart({ product: p, quantity: 1 }); }}
                      className="w-full py-4 bg-brand-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl transition-all hover:bg-brand-500 relative z-30"
                    >
                      {lang === 'bn' ? 'কার্টে যোগ করুন' : 'Deploy to Cart'}
                    </button>
                  </div>
                </div>
                ...
                {/* Content */}
                <div className="p-10">
                  <div className="text-[10px] font-black text-brand-500 uppercase tracking-widest mb-2">{p.category}</div>
                  <h3 className={`text-2xl font-black mb-4 transition-colors ${theme === 'dark' ? 'text-white' : 'text-slate-950'}`}>{p.name}</h3>
                  <div className="flex items-center gap-4">
                    <span className="text-2xl font-black text-brand-500">{currency}{(p.price || 0).toLocaleString()}</span>
                    {p.is_on_sale && (
                      <span className="text-sm font-bold text-slate-500 line-through">{currency}{(p.original_price || 0).toLocaleString()}</span>
                    )}
                  </div>
                </div>
              </motion.div>
