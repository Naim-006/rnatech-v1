  ...
  return (
    <div className="relative min-h-screen pb-24 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 py-20">
        ...
        <div className="flex flex-wrap justify-center gap-6 mb-12">
          {mainCategories.map((cat) => (
            <button
              ...
              className={`flex items-center gap-4 px-10 py-5 rounded-[2rem] font-black uppercase text-[12px] tracking-widest transition-all
                ${activeTab === cat.id
        ...
        {activeTab !== 'app' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap justify-center gap-3 mb-16">
            {getSubTabs().map((tab) => (
              <button
                key={tab}
            ...
            {filteredServices.map((service, idx) => (
              ...
              >
                ...
                <div className="p-8">
                  ...
                  <h3 className={`text-2xl font-black mb-4 group-hover:text-brand-500 transition-colors uppercase tracking-tight ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                    {service.name}
                  </h3>
                  <p className="text-slate-500 text-xs font-bold leading-relaxed line-clamp-2 mb-8">
                    {service.description}
                  </p>
                  <div className="flex items-center justify-between pt-6 border-t border-white/5">
                     <span className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-500">{t('portfolio_view_details')}</span>
        ...
        {/* Bottom CTA / Help */}
        <section className={`p-16 rounded-[4rem] border transition-all text-center relative overflow-hidden
          ...
          <h3 className={`text-4xl md:text-5xl font-black mb-6 tracking-tighter ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{t('services_custom_title')}</h3>
          <p className="text-slate-500 text-lg max-w-2xl mx-auto mb-10 font-medium leading-relaxed">
            {t('services_custom_desc')}
          </p>
          <button className="px-12 py-5 rounded-2xl bg-brand-600 hover:bg-brand-500 text-white font-black uppercase text-[12px] tracking-[0.2em] shadow-2xl shadow-brand-500/40 transition-all hover:-translate-y-2">
        ...
        {selectedService && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 40 }}
              className={`relative w-full max-w-4xl overflow-hidden rounded-[3rem] border shadow-3xl
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2">
                  <div className="h-full min-h-[400px] relative border-r border-white/5 flex flex-col">
                    <div className="flex-grow relative overflow-hidden">
                        ...
                        <div className="absolute bottom-12 left-12 right-12">
                           <div className="flex items-baseline gap-1 text-white">
                              ...
                              <span className="text-6xl font-black tracking-tighter">{(selectedService.base_price || 0).toLocaleString()}</span>
                           </div>
                           <p className="text-brand-500 font-black text-xs uppercase tracking-widest mt-2">Starting Investment</p>
                        </div>
                    </div>
                    
                    {selectedService.images && selectedService.images.length > 1 && (
                        <div className="p-6 bg-slate-950/50 flex gap-3 overflow-x-auto custom-scrollbar">
                 ...
                 <div className="p-12 flex flex-col justify-center">
                    ...
                    <h2 className={`text-4xl font-black mb-6 tracking-tighter uppercase ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                    ...
                    <div className="space-y-4 mb-12">
                       <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Global Fulfillment</h4>
                       <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-brand-500">
                             <ICONS.Zap className="w-5 h-5" />
                          </div>
                          <p className="text-xs font-black uppercase tracking-widest">{selectedService.timeline || '7-14 Days'} Timeline</p>
                       </div>
                       <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-cyan-500">
                             <ICONS.Shield className="w-5 h-5" />
                          </div>
                          <p className="text-xs font-black uppercase tracking-widest">{selectedService.tier || 'Enterprise'} Class Maintenance</p>
                       </div>
                    </div>

                    {selectedService.features && selectedService.features.length > 0 && (
                        <div className="mb-12">
                            <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Payload Features</h4>
                    ...
                    >
                      Manifest Inquiry <ICONS.WhatsApp className="w-5 h-5" />
                    </button>
                 </div>
              </div>
            </motion.div>
