...
const SignupPage = () => {
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.user) {
                alert("Registration successful! Please check your email inbox (and spam folder) for the verification link.");
                navigate('/login');
            }
        } catch (error: any) {
            alert(error.message);
        ...
        <div className={`min-h-screen flex items-center justify-center px-6 py-20 relative overflow-hidden transition-colors duration-500 ${theme === 'dark' ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
            ...
            >
                <div className="text-center mb-10">
                    ...
                    <h1 className="text-3xl font-black mb-3 tracking-tight">{t('auth_signup_title')}</h1>
                    <p className="text-slate-500 font-medium leading-relaxed">{t('auth_signup_subtitle')}</p>
                </div>
