    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      <div className="flex flex-col lg:flex-row gap-12">
        <aside className="w-full lg:w-64 space-y-2">
          <div className="p-6 md:p-8 bg-slate-900/50 rounded-2xl md:rounded-3xl border border-white/10 mb-6 md:mb-8 text-center shrink-0">
            <div className="w-16 h-16 bg-brand-600 rounded-full mx-auto mb-4 flex items-center justify-center text-xl font-black shadow-lg shadow-brand-600/20">
              {state.user?.email.charAt(0).toUpperCase()}
            </div>
            <h3 className="font-bold truncate text-sm">{state.user?.email}</h3>
            <p className="text-[10px] text-slate-500 font-bold mt-1 uppercase tracking-widest">{state.user?.role}</p>
          </div>
        ...
        <div className="flex-grow space-y-8">
          ...
          <div className="glass rounded-3xl border border-white/5 overflow-hidden">
            ...
            <div className="divide-y divide-white/5">
              {[
                { title: 'Project Update', desc: 'Nexus Node Pro Gateway v2 development started', date: '2 hours ago', status: 'In Progress' },
